...
class OTA:
    def __init__(self, verify=True, verbose=True, reboot=False, sha="", length=0, progress_cb=None):
        self.reboot = reboot
        ...
        self.writer = BlockDevWriter(self.part, verify, verbose, progress_cb)
        if sha or length:
...
def from_file(
    url: str, sha="", length=0, verify=True, verbose=True, reboot=True, username=None, password=None, headers={}, progress_cb=None
) -> None:
    with OTA(verify, verbose, reboot, progress_cb=progress_cb) as ota_update:
