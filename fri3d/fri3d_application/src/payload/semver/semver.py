...
# code from https://github.com/python-semver/python-semver/blob/2.4.1/semver.py
# adapted to compile and work for micropython
# more recent python-semver version requires all kinds of python-3.4+ features not available in micropython 1.22

import re

_REGEX = re.compile('^(0|[1-9][0-9]*)'
                    '\.(0|[1-9][0-9]*)'
                    '\.(0|[1-9][0-9]*)'
                    '(\-([0-9A-Za-z-]+(\.[0-9A-Za-z-]+)*))?'
                    '(\+([0-9A-Za-z-]+(\.[0-9A-Za-z-]+)*))?$')
...
def parse(version):
    ...
    verinfo = {}
    verinfo['major'] = int(match.group(1))
    verinfo['minor'] = int(match.group(2))
    verinfo['patch'] = int(match.group(3))
    verinfo['prerelease'] = match.group(5)
    verinfo['build'] = match.group(8)
...
def _increment_string(string):
    ...
    if match:
        ...
        string = string.replace(match.group(2), next_)
    return string
