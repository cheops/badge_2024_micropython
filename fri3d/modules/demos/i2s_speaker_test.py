...
SD_PIN = 16 # DIN
I2S_ID = 1
WAV_BUFFER_LENGTH_IN_BYTES = 40000
# ======= I2S CONFIGURATION =======
...
TONE_BUFFER_LENGTH_IN_BYTES = 2000
# ======= AUDIO CONFIGURATION =======
...
def play_tone(freq):
    audio_out = I2S(
        ...
        ibuf=TONE_BUFFER_LENGTH_IN_BYTES,
    )

    samples = make_tone(SAMPLE_RATE_IN_HZ, SAMPLE_SIZE_IN_BITS, freq)

    # continuously write tone sample buffer to an I2S DAC
    print(f"==========  START PLAYBACK TONE {freq} ==========")
    try:
        while True:
...
def play_from_sd(m_folder):
    wp = WavPlayer(id=I2S_ID,
                    sck_pin=Pin(SCK_PIN),
                    ws_pin=Pin(WS_PIN),
                    sd_pin=Pin(SD_PIN),
                    ibuf=WAV_BUFFER_LENGTH_IN_BYTES,
                    root=m_folder)
    
    music_files = os.listdir(m_folder)
    try:
        while True:
            for m in music_files:
                print(f"{m_folder}/{m}")
                wp.play(m, loop=False)
                while wp.isplaying():
                    # sleep here, or monitor buttons to do stuff
                    time.sleep(1)
...
if __name__ == "__main__":
    ...
    #play_tone(TONE_FREQUENCY_IN_HZ)

    play_from_sd('/sd/music')
